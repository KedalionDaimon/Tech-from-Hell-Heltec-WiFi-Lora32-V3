// THIS VARIANT IS FOR CONTROLLING LF-LINE-END TERMINALS AT 9600 BAUD.

// For "Heltec WiFi LoRa 32 (V3)", SF12@125kHz, codingrate 4/8:

// SF10, 250kHz, 30 chars = less than 1.085 seconds, delay(1100)

#include "LoRaWan_APP.h"
#include "Arduino.h"

#define CHARDELAY                                   1100
...
void Talker() {
  ...
  while (c != '\n') { // instead of ((c != '\n') && (c != '\r')), as LF ends anyway
        if (Serial.available()) {
          ...
          countchars++;
          delay(1 + CHARDELAY/CHARS_IN_PACKET); // slow typing
          // Serial.print(c);
